"use strict";
function __export(m) {
    for (var p in m) if (!exports.hasOwnProperty(p)) exports[p] = m[p];
}
Object.defineProperty(exports, "__esModule", { value: true });
__export(require("metaverse-rpc"));
__export(require("metaverse-rpc/lib/common/transports/WebWorker"));
__export(require("metaverse-rpc/lib/common/transports/WebSocket"));
__export(require("metaverse-rpc/lib/common/core/EventDispatcher"));
__export(require("./interfaces"));
__export(require("./events"));
__export(require("./MessageBus"));
const ScriptableScene_1 = require("./ScriptableScene");
exports.ScriptableScene = ScriptableScene_1.ScriptableScene;
const createElement_1 = require("./internal/createElement");
exports.createElement = createElement_1.createElement;
if (typeof global !== 'undefined') {
    global['createElement'] = global['createElement'] || createElement_1.createElement;
}
else if (typeof window !== 'undefined') {
    // tslint:disable-next-line:semicolon
    window['createElement'] = window['createElement'] || createElement_1.createElement;
}
//# sourceMappingURL=data:application/json;base64,